...
const ManageSupport = () => {
  ...
  const [isVipFilter, setIsVipFilter] = useState(false);
  ...
  const fetchConversations = async () => {
    try {
      const res1 = await getListConversation(
        id,
        token,
        unreadOnly,
        debouncedText,
    ...
    const fetchUnclaimedConversations = async () => {
      try {
        ...
        const processed = res.data.map(
          (item) => getOtherParticipants([item])[0]
        );
        setConversation((prev) => {
          const newList = [...prev];
    ...
    <div className="admin-chat-container">
          ...
          <div className="admin-chat-box-right-settings-select">
            <FormControl sx={{ minWidth: 200 }} size="small">
              ...
              >
                ...
                <MenuItem value="vip">Hỗ trợ VIP</MenuItem>
              </Select>
            </FormControl>
          </div>
        </div>
        <div
          className="admin-chat-box-right-body"
          style={{ overflowY: "auto", maxHeight: "400px" }}
        >
          {Array.isArray(conversation) &&
            conversation.map((msg) => {
              const isUnread = Array.isArray(msg?.readBy)
              ...
              return (
                ...
                >
                  ...
                  <div className="admin-chat-card-content">
                    <div
                      className={`admin-chat-card-name ${isUnread ? "unread-text" : ""}`}
                    >
                      {truncateMessage(msg?.username, 30)}
                      {msg.isVipUser && <span className="vip-badge">VIP</span>}
                    </div>
                    <div
                      className={`admin-chat-card-message ${isUnread ? "unread-text" : ""}`}
                    >
      ...
      <div className="admin-chat-box-left">
          ...
          {selectedChat ? (
            <>
              <Avatar
                ...
                className="admin-chat-card-avatar-left"
              />
              <div className="admin-chat-card-content-left">
                <div className="admin-chat-card-name-left">
                  {truncateMessage(selectedChat.username, 30)}
                </div>
                <div className="admin-chat-card-online">
                  <span
                    style={{
                      ...
                      height: "8px",
                      borderRadius: "50%",
                      backgroundColor: onlineUsers.includes(selectedChat.userId)
                        ? "green"
                        : "gray",
                      marginRight: "5px",
                    }}
              ...
              <MoreVertIcon
                style={{ marginLeft: "auto", alignContent: "center" }}
              />
            </>
          ) : (
            <div className="admin-chat-card-content">
              <div className="admin-chat-card-name">
        ...
        <div className="admin-chat-box-left-content" ref={chatRef}>
          {messagesChat
            ...
            .map((msg) => (
              <div
                key={msg.id}
                className={`admin-message ${msg.sender === id ? "sent" : "received"}`}
              >
        ...
        <div className="admin-chat-box-left-footer">
          <div className="admin-chat-box-left-footer">
            <div
              className={`admin-chat-box-left-footer-media ${showIcons ? "visible" : "hidden"}`}
            >
              <InsertPhotoIcon
                sx={{ color: "#63ab45" }}
          ...
          <div
            className="admin-chat-box-left-input-container"
            style={{ flex: showIcons ? 8 : 10 }}
          >
            <input
              ref={inputRef}
              type="text"
